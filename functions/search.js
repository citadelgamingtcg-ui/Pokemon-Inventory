...
// Endpoint: /search?q=Riolu&number=215
// NOTE: files in /functions must export onRequest. The Netlify copy in
// netlify/functions/search.js keeps exports.handler — don't copy one over the other.
...
export async function onRequest(context) {
  ...
  const TCGAPI_KEY = env.TCGAPI_KEY;   // Workers has no process.env
  ...
  const q = (params.get('q') || '').trim().replace(/\s*\bjapanese\b\s*/ig, ' ').trim();
  const number = (params.get('number') || '').split('/')[0].replace(/^0+/, '');
  ...
  try {
    ...
    return new Response(JSON.stringify({ data: matches, source: 'tcgcsv', cat, note: tcgapiNote }), { status: 200, headers });
  } catch (err) {
