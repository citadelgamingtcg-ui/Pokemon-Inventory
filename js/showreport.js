...
(function () {
  ...
  /**
   * Rebuild what inventory was worth at a past moment. Forgetting to start a
   * show is normal when the table is busy; the snapshot is recoverable because
   * every card that left or arrived since then is timestamped.
   *
   *   opening = what's on hand now
   *           + what left after that moment (at market)
   *           - what arrived after that moment (at market)
   */
  function reconstructOpening(cards, sinceTs) {
    const now = inventoryValueCents(cards);
    const mkt = x => c(x.tcgPrice) * (parseInt(x.qty, 10) || 1);

    const leftSince = (cards || [])
      .filter(x => exitKind(x) && exitTime(x) >= sinceTs)
      .reduce((s, x) => s + mkt(x), 0);

    const gainedSince = (cards || [])
      .filter(x => (x.acquiredAt || x.addedAt || 0) >= sinceTs && !exitKind(x))
      .reduce((s, x) => s + mkt(x), 0);

    return {
      openingCents: Math.max(0, now + leftSince - gainedSince),
      nowCents: now,
      leftCents: leftSince,
      gainedCents: gainedSince,
      leftCount: (cards || []).filter(x => exitKind(x) && exitTime(x) >= sinceTs).length,
      gainedCount: (cards || []).filter(x => (x.acquiredAt || x.addedAt || 0) >= sinceTs && !exitKind(x)).length
    };
  }

  /** Earliest recorded activity today — a sensible default start time. */
  function earliestActivity(cards, txs, sinceTs) {
    const times = [];
    for (const x of cards || []) {
      const t = exitTime(x);
      if (t && t >= sinceTs) times.push(t);
      const a = x.acquiredAt || x.addedAt;
      if (a && a >= sinceTs) times.push(a);
    }
    for (const t of txs || []) if (t.at && t.at >= sinceTs) times.push(t.at);
  ...
  const API = { inventoryValueCents, showReport, reportWarnings, reconstructOpening, earliestActivity };
