...
(function () {
  ...
  // Cloudflare and browsers cache /data/* for a day (see _headers) because the
  // file is large and usually static. That backfires the moment prices.json
  // actually changes — the app code updates instantly (HTML/JS are stamped
  // per version) but the data silently stays stale for up to 24h. Stamping
  // the fetch itself with APP_VERSION forces a fresh copy on every deploy.
  const DATA_VERSION = (typeof window !== 'undefined' && window.APP_VERSION) || Date.now();
  const DATA_URL = 'data/prices.json?v=' + DATA_VERSION;
  ...
  async function loadJP() {
    ...
    jpLoading = (async () => {
      try {
        const res = await fetch('data/prices-jp.json?v=' + DATA_VERSION);
  ...
  function toCard(r, cond) {
    ...
    return {
      ...
      // pid (TCGplayer product id) is tried first since it's the primary source;
      // img (a direct URL from the pokemontcg.io fallback) covers records
      // TCGCSV hasn't caught up on yet — different card, different CDN, same field.
      image_url: r.pid ? `https://tcgplayer-cdn.tcgplayer.com/product/${r.pid}_200w.jpg` : (r.img || null),
